#!/usr/bin/env npx tsx
/**
 * Generate repository summary for Superintendent
 * Usage: npx tsx scripts/generate-repo-summary.ts [path]
 *
 * This creates a .superintendent/repo-summary.json file with:
...
const REPO_SUMMARY_PATH = path.join(SUPERINTENDENT_DIR, 'repo-summary.json');
const LEGACY_REPO_SUMMARY_PATH = path.join(PROJECT_ROOT, '.superintendent-repo-summary.json');

// Ensure .superintendent directory exists
function ensureSuperintendentDir(): void {
  if (!fs.existsSync(SUPERINTENDENT_DIR)) {
    fs.mkdirSync(SUPERINTENDENT_DIR, { recursive: true });
  }
}
...
const EXCLUDED_DIRS = new Set([
  'node_modules', '.git', '.superintendent', 'dist', 'build', 'coverage',
...
const DOCUMENTATION = `
This file contains context about your repository for Superintendent.
...
- manual: Edit these fields to provide domain knowledge that cannot be
          automatically detected. This helps Superintendent ask better questions
...
function analyzeRepository(workDir: string): EnhancedRepoSummary {
  ...
  const newSummaryPath = path.join(workDir, '.superintendent', 'repo-summary.json');
  const legacySummaryPath = path.join(workDir, '.superintendent-repo-summary.json');
