#!/usr/bin/env npx tsx
/**
 * One-time script to create Superintendent trigger labels in Linear
 *
...
const SUPERINTENDENT_DIR = path.join(PROJECT_ROOT, '.superintendent');
const TOKEN_PATH = path.join(SUPERINTENDENT_DIR, 'token.json');

// Label group configuration
const LABEL_GROUP = {
  name: 'superintendent',
  color: '#6366f1', // Indigo
  description: 'Superintendent trigger labels',
};
...
async function main() {
  console.log('Creating Superintendent trigger labels...\n');
  ...
  // Step 1: Check if superintendent exists and whether it's team or workspace level
