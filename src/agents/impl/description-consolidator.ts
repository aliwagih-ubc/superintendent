...
export const DescriptionConsolidatorInputSchema = z.object({
  ...
  comments: z.array(z.object({
    body: z.string(),
    isFromSuperintendent: z.boolean(),
    createdAt: z.date(),
  })),
});
...
export class DescriptionConsolidatorAgent extends BaseAgent<DescriptionConsolidatorInput, DescriptionConsolidatorOutput> {
  ...
  private buildContext(input: AgentInput<DescriptionConsolidatorInput>): string {
    ...
    // Separate Superintendent questions from human answers
    const superintendentComments = comments.filter(c => c.isFromSuperintendent);
    const humanComments = comments.filter(c => !c.isFromSuperintendent);
...
## Questions Asked by Superintendent
`;
