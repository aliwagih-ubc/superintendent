...
${readinessResult.reasoning}`;
    ...
    if (existingComments.length > 0) {
      const superintendentComments = existingComments.filter((c) => c.isFromSuperintendent);
      const humanComments = existingComments.filter((c) => !c.isFromSuperintendent);

      if (superintendentComments.length > 0) {
        context += '\n\n## Previous Superintendent Questions\n';
        for (const comment of superintendentComments) {
...
Focus on remaining gaps and unanswered questions.`;
  ...
  private filterAnsweredQuestions(
    ...
    const humanResponses = existingComments
      .filter((c) => !c.isFromSuperintendent)
