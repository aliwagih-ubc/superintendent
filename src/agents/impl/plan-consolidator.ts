...
Format the plan in markdown. Be thorough and specific.`;
  ...
  private buildConversation(comments: Array<{ body: string; isFromSuperintendent: boolean; createdAt: Date }>): string {
    // Sort by creation time
    const sorted = [...comments].sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime());

    let conversation = '';
    for (const comment of sorted) {
      const speaker = comment.isFromSuperintendent ? 'Superintendent' : 'User';
