...
// Helper to check if a comment is from Superintendent
function isSuperintendentComment(user?: { id?: string; isMe?: boolean } | null): boolean {
...
export class QueueProcessor {
  ...
  private async handleRefine(task: LinearQueueItem): Promise<void> {
    ...
    // If forceAskQuestions is set (from "superintendent/clarify" label), override readiness to force questions
    if (forceAskQuestions) {
      logger.info({ ticketId: task.ticketIdentifier }, 'Force asking questions (triggered via superintendent/clarify label)');
      readiness = {
        ...readiness,
        ready: false,
    ...
    const input: AgentInput<TicketRefinerInput> = {
      ...
      data: {
        ...
        existingComments: comments.map((c) => ({
          ...
          isFromSuperintendent: isSuperintendentComment(c.user),
        })),
        codebaseContext,
  ...
  /**
   * Handle the @superintendent rewrite command - consolidate discussion into updated description.
  ...
  private async handleConsolidate(task: LinearQueueItem): Promise<void> {
    ...
    logger.info(
      ...
      'Consolidating discussion into description (@superintendent rewrite)'
    );
  ...
  /**
   * Handle the @superintendent work command - start execution directly without approval flow.
  ...
  private async handleExecuteDirect(task: LinearQueueItem): Promise<void> {
    ...
    logger.info(
      { ticketId: task.ticketIdentifier },
      'Starting direct execution (@superintendent work)'
    );
    ...
    if (isRestart && previousSessionTimestamp) {
      ...
      // Exclude Superintendent comments - we only want user feedback
      const newComments = allComments
        .filter(c => new Date(c.createdAt) > cutoffDate)
        .filter(c => !isSuperintendentComment(c.user))
        .map(c => ({
          body: c.body,
          createdAt: c.createdAt,
          isFromUser: true,
    ...
    const promptInput: AgentInput<PromptGeneratorInput> = {
      ...
      data: {
        ...
        constraints: {
          branchNaming: `superintendent/${ticket.identifier.toLowerCase()}`,
        },
    ...
    logger.info(
      ...
      'Enqueueing Claude Code execution (direct from @superintendent work)'
    );
  ...
  private async handleCheckResponse(task: LinearQueueItem): Promise<void> {
    ...
    // New flow: scheduler triggers 'evaluate' directly, which handles everything
    //
  ...
  /**
   * Handle the @superintendent plan command - enter planning mode with Claude Code.
  ...
  private async handlePlan(task: LinearQueueItem): Promise<void> {
    ...
    logger.info(
      { ticketId: task.ticketIdentifier },
      'Starting planning mode (@superintendent plan)'
    );
    ...
    if (!extractorResult.success || !extractorResult.data) {
      ...
      await linearClient.addComment(
        task.ticketId,
        `Planning analysis complete. Use \`@superintendent work\` to start implementation.\n\n_Note: Could not extract structured questions._`
      );
      linearQueue.complete(task.id, result.data);
      return;
    }
...
${ticket.description || ''}`;
    ...
    if (questions.length > 0) {
      ...
      await linearClient.addComment(
        task.ticketId,
        '✅ Planning complete! Please answer the questions above, then use `@superintendent work` to start implementation.'
      );
      ...
      await linearClient.addComment(
        task.ticketId,
        '✅ Planning complete! Use `@superintendent work` to start implementation.'
      );
    }
  ...
  private async handleConsolidatePlan(task: LinearQueueItem): Promise<void> {
    ...
    const hasPlanningQuestions = comments.some(
      (c) => isSuperintendentComment(c.user) && c.body.includes('Planning Question')
    );
    ...
    const hasHumanResponses = comments.some(
      (c) => !isSuperintendentComment(c.user) && c.createdAt > firstPlanningQuestion.createdAt
    );
...
${prRetryPromptResult.data.prompt}`;
  ...
  private async requestApproval(
    ...
    const hasExistingApprovalRequest = comments.some(
      (c) => isSuperintendentComment(c.user) && c.body.includes('React with 👍 to approve')
    );
   ...
   * 1. There are Superintendent questions (comments with ❗/❓/💭)
  ...
  private async maybeConsolidateDescription(
    ...
    // Find Superintendent question comments (unresolved ones)
    ...
    const questionComments = comments.filter(
      (c) => isSuperintendentComment(c.user) && (
    ...
    // Also check for human reply comments (not from Superintendent)
    ...
    if (questionCommentIds.length > 0) {
      ...
      logger.info(
        ...
        'Resolved Superintendent question comments after consolidation'
      );
    }
  ...
  private hasUnansweredQuestions(
    ...
    // Human comments are those not from Superintendent
    const humanCommentAfterQuestion = comments.some((c) =>
      !isSuperintendentComment(c.user) && c.createdAt > mostRecentQuestion.createdAt
    );
